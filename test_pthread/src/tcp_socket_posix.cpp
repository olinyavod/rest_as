...
tcp_socket::tcp_socket()
    : port_(0), host_(""), socket_fd_(-1), is_listening_(false), is_server_socket_(false), is_connected_(false) {
    memset(&address_, 0, sizeof(address_));
}

tcp_socket::tcp_socket(int port, const std::string& host)
    : port_(port), host_(host), socket_fd_(-1), is_listening_(false), is_server_socket_(true), is_connected_(false) {
    memset(&address_, 0, sizeof(address_));
}
...
std::expected<std::unique_ptr<tcp_socket>, std::string> tcp_socket::accept_connection() {
    ...
    int new_socket = accept(socket_fd_, (struct sockaddr*)&address_, (socklen_t*)&addrlen);
    if (new_socket < 0) {
...
std::expected<void, std::string> tcp_socket::send(std::istream& data) {
    ...
    while (data.read(buffer, sizeof(buffer)) || data.gcount() > 0) {
        ssize_t bytes_to_send = data.gcount();
        ssize_t bytes_sent = ::send(socket_fd_, buffer, bytes_to_send, 0);
        if (bytes_sent < 0) {
...
tcp_socket::tcp_socket(int socket_fd, const struct sockaddr_in& address, bool is_server_socket)
